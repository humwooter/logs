...
struct NewScheduleView: View {
    let days: [Date]
    let hours = Array(0..<24).map { "\($0 % 12 == 0 ? 12 : $0 % 12) \($0 < 12 ? "AM" : "PM")" }
    
    // Date formatter to display day and date on the X-axis
    ...
    var body: some View {
        ScrollView([.horizontal, .vertical], showsIndicators: false) {
            VStack(alignment: .leading, spacing: 0) {
                // Top row with day headers
                HStack(spacing: 0) {
                    // Empty top-left corner space for Y-axis hour labels
                    Text("")
                ...
                // Rows with hour labels and placeholders for schedule items
                ForEach(hours.indices, id: \.self) { hourIndex in
                    HStack(spacing: 0) {
                        // Hour labels on the left (Y-axis)
                        Text(hours[hourIndex])
                            .frame(width: 80, height: 50)
                            .background(Color.gray.opacity(0.1))
                            .border(Color.gray)
                        
                        // Placeholder cells for each hour slot
...
//                        }
    ...
    @ViewBuilder
    private func scheduleView() -> some View { //working dayColumnHeader (fixed x axis)/column headers
...
//   
//    @ViewBuilder
//    private func scheduleView() -> some View { //working timelinecolumn (fixed y axis/row headers)
//        GeometryReader { geometry in
//            let availableWidth = geometry.size.width - timeLabelWidth
//            let dayColumnCount = max(selectedDates.count, 1)
//
...
//
//    @ViewBuilder
//    private func scheduleView() -> some View { //working timelinecolumn
//        GeometryReader { geometry in
//            let availableWidth = geometry.size.width - timeLabelWidth
//            let dayColumnCount = max(selectedDates.count, 1)
//
...
//
    ...
    @ViewBuilder
    private func dayHeaders(calculatedDayColumnWidth: CGFloat) -> some View {
        HStack {
            ForEach(selectedDates, id: \.self) { date in
